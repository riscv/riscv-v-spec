...
`vsew` is a 3-bit `WARL` field that can be expanded to match the
`vew[5:0]` expanded width encoding in <<vewn>>. `vsew` stands for
"Vector Single Element Width".

.`vsew[5:3]` Base Encoding
[#vsew,options="header"]
|===
|`vsew[5:3]` | Width | Required in base
|001 | 8 | All
|010 | 16 | All
|011 | 32 | All
...
source element.  Vector stores for a data type wider than `vsew` cause
