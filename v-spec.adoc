...
vwsmaccu.vv vd, vs2, vs1, vm # vd[i] = clipu(vd[i]+((vs2[i]*vs1[i]+round)>>SEW/2))
vwsmaccu.vx vd, vs2, rs1, vm # vd[i] = clipu(vd[i]+((vs2[i]*x[rs1]+round)>>SEW/2))

# Widening signed-integer scaled multiply-accumulate
vwsmacc.vv vd, vs2, vs1, vm  # vd[i] = clip(vd[i]+((vs2[i]*vs1[i]+round)>>SEW/2))
vwsmacc.vx vd, vs2, rs1, vm  # vd[i] = clip(vd[i]+((vs2[i]*x[rs1]+round)>>SEW/2))

# Widening unsigned-integer scaled multiply-subtract
vwsmsacu.vv vd, vs2, vs1, vm # vd[i] = clipu(vd[i]-((vs2[i]*vs1[i]+round)>>SEW/2))
vwsmsacu.vx vd, vs2, rs1, vm # vd[i] = clipu(vd[i]-((vs2[i]*x[rs1]+round)>>SEW/2))

# Widening signed-integer scaled  multiply-subtract
vwsmsac.vv vd, vs2, vs1, vm  # vd[i] = clip(vd[i]-((vs2[i]*vs1[i]+round)>>SEW/2))
vwsmsac.vx vd, vs2, rs1, vm  # vd[i] = clip(vd[i]-((vs2[i]*x[rs1]+round)>>SEW/2))
