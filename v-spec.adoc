...
instruction mnemonic and operand suffix, for instructions that follow
the <<Vector Arithmetic Operand encoding>>, or similar operand encoding.
For vector loads and stores the `.v` suffix  is replaced with the `.s` suffix.
...
[source]
----
                     Width xv  Mem     Reg       opcode uoffset scale
                     [2:0]     Bits    Bits
...
Vector single-width   111  0  vl*SEW   vl*SEW    VxE     1

Mem bits is the size of element moved in memory
Reg bits is the size of element accessed in register
uoffset scale is the amount by which the unsigned immediate is multiplied to obtain a byte offset
----
...
An offset is constructed from bits[31:30] or bits[11:10] for non unit-strided
loads and stores respectively. For unit-strided loads and stores an offset is
constructed from bits[31:30,24:20] and bits[11:10,24:20] respectively.
This unsigned offset that is added to the base register during vector
addressing.  The offset is scaled according to the last column of the previous
<<load-store-types, table>> (effective offset = uoffset * scale), such that for
fixed-width elements the offset is scaled by the element size.  For
dynamic-width elements, the offset is not affected by the `vtype` setting to
avoid having a dependency between address generation and dynamic `vtype` value.

[source]
----
 Use of 12b immediate field in vector load/store instruction encoding

  31 30 29 28 27 26 25 24 23 22 21 20 11 10  9  8  7  Load   immediate bits
       funct5    m1 m0   off/str/idx      dest        Field
  31 30 29 28 27 26 25 24 23 22 21 20 11 10  9  8  7  Store   immediate bits
       value     m1 m0   off/str/idx     funct5       Field
