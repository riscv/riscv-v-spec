...
vector register, `v<n>`, is configured via 16 bits of vector type
configuration state `vtype<n>`, which can be accessed via vector
...
well as the bitwidth and numeric representation of each element.
...
6-bit element bit-width field `vew<n>`, held in the `vcfg<n>` CSRs.
...
Each vector register, `v<n>`, has a 6-bit element width register,
`vew<n>`, to specify the number of bits for each element of the current
