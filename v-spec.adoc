...
XLEN-width CSRs (`vtype`, `vl`, and `vlmax`) to a base scalar RISC-V
ISA.
...
The `vlmul?max` versions are used here to reduce LMUL for shorter
application vector lengths.  The same AVL must be supplied to both the
16-bit/LMULMAX=4 and the 32-bit/LMULMAX=8 `vsetvli` instructions to
obtain correct results.

== Vector instruction formats

Vector instructions can have scalar or vector source operands and
produce scalar or vector results, and almost all vector instructions
can be performed conditionally under a mask.
...
vectors.  Instructions producing scalars are not maskable.  Element
operations that are masked off do not modify the destination vector
register element and never generate exceptions.
...
vm[1:0]
  ...
  00    vector, only where v0[i].LSB = 0
  01    vector, only where v0[i].LSB = 1
  10    scalar result, no masking
  11    vector result, no masking
----

Scalar operations are written in assembler with a `.s` after the
opcode.  Vector masking is written as another vector operand, with
`.t` or `.f` indicating if operation occurs when `v0[0]` is `1` or `0`
respectively.  If no masking operand is specified, unmasked vector
execution (`m=11`) is assumed.

[source,asm]
----
    vop    v1, v2, v3, v0.f  # enabled where v0[i].LSB=0, m=00
    vop    v1, v2, v3, v0.t  # enabled where v0[i].LSB=1, m=01
    vop.s  v1, v2, v3        # scalar operation,          m=10
    vop    v1, v2, v3        # unmasked vector operation, m=11
----

In the listings below, `vop v1, v2, v3, vm` implies one of the
non-scalar forms.
...
elements in the destination unmodified reduces instruction count in more
...
#
  loop:
    ...
    vmerge.vi v4, v4, 1           # Initialize v4.
...
=== Scalar Vector Memory Instructions

When the vm[1:0] field is set to scalar, the vector load/store
instructions move a single value between element 0 of the vector
register and memory using the same address that would be used for
element 0 of a vector operation.

NOTE: As scalar accesses only access element 0, a strided vector
access has the same effect as a unit-stride access.  Removing this
redundancy in the encoding by making strided scalar accesses reserved
would slightly complicate control logic, so the redundancy was left in
place.  However, unit-stride is the canonical address mode to use when
encoding a scalar vector memory instruction.
...
the fourth encoding moves variable-sized memory elements of SEW bits.
...
=== Vector instructions assembler syntax

==== Unit-stride instruction assembler syntax
...
----
    ...
    # Scalar unit-stride loads

    vlb.s vd, offset(rs1)      # signed 8b scalar load into element 0
    vlh.s vd, offset(rs1)      # signed 16b scalar load into element 0
    vlw.s vd, offset(rs1)      # signed 32b scalar load into element 0

    vlbu.s vd, offset(rs1)     # unsigned 8b scalar load into element 0
    vlhu.s vd, offset(rs1)     # unsigned 16b scalar load into element 0
    vlwu.s vd, offset(rs1)     # unsigned 32b scalar load into element 0

    vle.s vd, offset(rs1)      # SEW scalar load into element 0
    ...
    vsh.s   vs3, offset(rs1)      # Scalar 16b store from element 0
    vsw.s   vs3, offset(rs1)      # Scalar 32b store from element 0
    vse.s   vs3, offset(rs1)      # Scalar SEW store from element 0
----

==== Vector strided instructions assembler syntax
...
----
    ...
    # Scalar strided load (redundant with unit-stride)
    vlsb.s    vd, offset(rs1), rs2  # 8b
       ... etc

    # Scalar strided store (redundant with unit-stride)
    vssb.s   vs3, offset(rs1), rs2  # 8b
       ... etc
----

==== Vector indexed instructions assembler syntax
...
----
    ...
    vlxb.s   vd,  offset(rs1), vs2     # 8b 
    vlxh.s   vd,  offset(rs1), vs2     # 16b
    vlxw.s   vd,  offset(rs1), vs2     # 32b

    vlxbu.s   vd,  offset(rs1), vs2    # 8b 
    vlxhu.s   vd,  offset(rs1), vs2    # 16b
    vlxwu.s   vd,  offset(rs1), vs2    # 32b

    vlxwe.s   vd,  offset(rs1), vs2    # SEW
           
    # Scalar ordered-indexed store instructions
    vsxb.s    vs3, offset(rs1), vs2    # 8b
    vsxh.s    vs3, offset(rs1), vs2    # 16b
    vsxw.s    vs3, offset(rs1), vs2    # 32b
    vsxe.s    vs3, offset(rs1), vs2    # SEW

    # Scalar unordered-indexed store instructions not supported
----

NOTE: Dropped reverse-ordered scatter for now, can use rgather to
reverse index order.  Encoding space would allow, assembler syntax
could be `vsrxb.v.

==== Unit-stride first-fault versions
...
=== Arithmetic instructions assembler syntax
