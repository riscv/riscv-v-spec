...
31       26  25   24      20 19      15 14   12 11      7 6     0
  funct6   | vm  |   vs2    |    vs1   | funct3|  vd/rd  |1010111| OP-V.vv
  funct6   | vm  |   vs2    |    rs1   | funct3|  vd/rd  |1010111| OP-V.vs
  funct6   | vm  |   vs2    |   simm5  | funct3|  vd/rd  |1010111| OP-V.vi
     6        1        5          5        3        5        7
----
...
x 0 0   Int vector-scalar        GPR x register rs1
x 0 1   Int vector-immediate     simm[4:0]
x 1 0   Int vector-vector              -
...
[source]
----
    vfredsum.v  vd, vs2, vs1, vm # Ordered sum
    vfredusum.v vd, vs2, vs1, vm # Unordered sum
    vfredmax.v  vd, vs2, vs1, vm
    vfredmin.v  vd, vs2, vs1, vm
----

The `vfredsum` instruction performs the floating-point sum reduction
in element order, while the `vfredusum` is allowed to perform the
reduction in any order.  The result of `vfredsum` must correspond to
some ordering of individual sequential reduction operations.
...
Widening forms of the sum reduction are provided that read and write a
double-width reduction result.

[source]
----
 vwfredsum.v  vd, vs2, vs1, vm  # Ordered reduce 2*SEW = sum(SEW) + 2*SEW
 vwfredusum.v  vd, vs2, vs1, vm # Unordered reduce  2*SEW = sum(SEW) + 2*SEW
----
...
[source]
----
    vmpopc rd, vs1, vm # rd = sum_i ( vs1[i].LSB )
----
...
[source]
----
 vmpopc x10, v1, v0.t # rd = sum_i ( v1[i] && v0[i] )
----
...
The instruction can be masked but the destination vector register
group cannot overlap the mask register unless LMUL=1, otherwise an
...
NOTE: The `vid.v` instruction can be implemented using the same
datapath as `vmiota.v` but with an implicit set mask source.

== Vector Permutation Instructions

A range of permutation instructions are provided to move elements
around within a vector registers.
...
=== Vector Register Scatter Instruction

This instruction reads elements from a source vector, then writes to
the destination vector register at the locations given by a second
source element index vector.  The values in the index vector are
treated as unsigned integers.

[source]
----
vrscatter.vv vd, vs1, vs2, vm # vd[vs2[i]] = vs1[i]
----
...
Precise vector traps, require that:
 ...
 - no operations within the trapping vector instruction affecting elements at or following the `vstart` CSR have altered architectural state
...
strlen:
    mv a3, a0             # Save start
loop:
    setvli a1, x0, vsew8, vreg4  # Vector of bytes, only use 4 vregs
