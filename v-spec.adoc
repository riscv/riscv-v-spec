...
`vemaxw` causes the entire vector unit state to be zeroed, even if
the value in `vemaxw` is unchanged.
...
. `vl \<= MAXVL`
. `vl = AVL` if `AVL \<= MAXVL`
. `vl` monotonically increases for `MAXVL > AVL` (but need not strictly increase)
...
types and operations implied by the supported scalar types as defined by
tables <<velemtypes-int>> and <<velemtypes-fp>>.
...
Supported data element formats depend on base integer ISA
and supported floating-point extensions.  `Ix` indicates a signed
integer of `x` bits, `Ux` indicates an unsigned integer of `x` bits,
...
preprocess the input to get acorrect result from a mask (e.g., zero masked
elements before sum).
[source,asm]
----
    vredsum vd.s, vs1
