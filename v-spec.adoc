...
extend::
  ...
  register.
extract::
  Use the lower `N` bits of the value in the source register, where `N`
  is the precision of the operation.
  For example, `vfadd.s` with a 64-bit operand uses the lower 32 bits
  and ignores the upper 32 bits.
