...
multiple destination vector register groups.  The low three bits of
`*umop` indicate the number of memory fields (_NFIELDS_) in a
structure from 1-8. The width encoding gives the size of the memory
fields, which must be homogeneous.

NOTE: For structures with heterogeneous-sized fields, software can
unpack fields using additional instructions after the
array-of-structures load brings the values into the separate vector
registers.
...
[source,asm]
----
    vlaos2b.v vd, offset(rs1), vm   # Load vector of 2*1-byte structure into destination vd, vd+1 when LMUL=1
...
When LMUL=1, each field is held in successively numbered vector
register numbers.  When LMUL > 1, each field is held in a vector
register group constructed from LMUL consecutive vector registers.

The value of LMUL must be such that LMUL multiplied by the number of
NFIELDS is less than or equal to 8, otherwise an illegal instruction
exception is raised.

[source]
----
    vsetvli x0, a0, vsew32,vlmul4
    vloas2w.v v8, (a1)  # <v8,v9,v10,v11> holds first field, <v12,v13,v14,v15> holds second field
----

The base vector register specifier given in the vector load or store
instruction must obey the vector register number constraints for a
vector register group with an effective LMUL of NFIELDS*LMUL (i.e.,
with NFIELDS=3 and LMUL=2, vector register number must be a multiple
of 8), otherwise an illegal instruction exception is raised.
...
vsetvli t0, a0, ve32,vediv4  # Vectors of 32-bit elements, divided into byte sub-elements
...
vsetvli t0, a0, vsew32,vlmul1,vediv2  # Vectors of 32-bit elements, divided into 16b sub-elements
...
vsetvli t0, a0, vsew64,vlmul1,vediv4  # Vectors of 64-bit elements, divided into 16b sub-elements
vfdot.vv v1, v2, v3
