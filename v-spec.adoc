...
fields in memory to and from consecutively numbered vector register
groups.
...
numbers.  If LMUL > 1, an illegal instruction exception will be
raised.
...
----
    # Format
    vlseg<nf>{b,h,w,e}.v vd, (rs1),  vm   # Unit-stride segment load template
    vsseg<nf>{b,h,w,e}.v vs3, (rs1), vm  # Unit-stride segment store template

    # Examples
    vlseg2b.v vd, (rs1), vm   # Load vector of 2*1-byte segments into vd, vd+1
    vlseg3b.v vd, (rs1), vm   # Load vector of 3*1-byte segments into vd, vd+1
...
----
    ...
    vlsegff<nf>{b,h,w,e}.v vd, (rs1),  vm  # Unit-stride fault-only-first segment load template
    vssegff<nf>{b,h,w,e}.v vs3, (rs1), vm  # Unit-stride fault-only-first segment store template
----
...
----
    # Format
    vlsseg<nf>{b,h,w,e}.v vd, (rs1), rs2, vm  # Strided segment load
    vssseg<nf>{b,h,w,e}.v vs3, (rs1), rs2, vm # Strided segment store
...
----
    # Format
    vlxseg<nf>{b,h,w,e}.v vd, (rs1), vs2, vm  # Indexed segment load
    vsxseg<nf>{b,h,w,e}.v vs3, (rs1), vs2, vm # Indexed segment store

    # Examples
    vlxseg3b.v v4, (x5), v3   # Load bytes at addresses x5+v3[i]   into v4[i],
