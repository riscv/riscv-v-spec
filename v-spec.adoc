...
.Table `vemaxw[5:3]` Base Encoding
...
being used.
It can be hardwired to zero if vtypes not supported, as in the base
vector extension.
...
Each architectural vector register `v<n>` is configured via 16 bits of
...
.Table `vshape<n>` Extended Encoding
...
[NOTE]
Other encodings are reserved for additional shapes.
...
.Table `verep<n>` Extended Encoding
...
[NOTE]
Other encodings are reserved for additional representations.
...
[NOTE]
Other encodings are reserved for additional element widths.
...
for a data type wider than `vemaxw` cause an illegal instruction exception.
