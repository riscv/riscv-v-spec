...
loop:
    ...
    vmul.vx  v8, v8, v1     # 32b multiply result
...
loop:
    ...
    vlh.v v4, (a1)          # Get 16b vector
      slli t1, a3, 1
      add a1, a1, t1        # Bump pointer
    vwmul.vx v8, v4, v1     # 32b in <v8--v15>
...
pseudo-ops `vwcvt.x.x.v vd,vs,vm = vwadd.vx vd,vs,x0,vm` and `vwcvtu.x.x.v
vd,vs,vm = vwaddu.vx vd,vs,x0,vm`.
...
# vsgteu.vx vd, vs2, rs1, vm    # Vector-scalar
# Set if greater than or equal, signed
# vsgte.vx vd, vs2, rs1, vm    # Vector-scalar
----
...
[source]
----
Comparison      Assembler Mapping                                Assembler Pseudo-op

va < vb         vslt{u}.vv vd, va, vb, vm                        
va <= vb        vslte{u}.vv vd, va, vb, vm
va > vb         vslt{u}.vv vd, vb, va, vm                        vsgt{u}.vv vd, va, vb, vm
va >= vb        vslte{u}.vv vd, vb, va, vm                       vsgte{u}.vv vd, va, vb, vm

va < x          vslt{u}.vx vd, va, x, vm
va <= x         vslte{u}.vx vd, va, x, vm
va > x          vsgt{u}.vx vd, va, x, vm
va >= x         see below

va < i          vslte{u}.vi vd, va, i-1, vm                      vslt{u}.vi vd, va, i, vm
va <= i         vslte{u}.vi vd, va, i, vm
va > i          vsgt{u}.vi vd, va, i, vm
va >= i         vsgt{u}.vi vd, va, i-1, vm                       vsgte{u}.vi vd, va, i, vm
...
false). Similarly, `vsgte{u}.vi` is not provided and the comparison is
implemented using `vsgt{u}.vi` with the immediate decremented by one.
The resulting effective `vgte.vi` range is -15 to 16, and the
resulting effective `vgteu.vi` range is 1 to 16 (Note, `vgteu.vi` with
...
but assembler pseudo-op sequences can be provided for cases where the
...
unmasked va >= x
  ...
  pseudo-op: vsgte{u}.vx vd, va, x
  expansion: vslt{u}.vx vd, va, x; vmnand.vv vd, vd, vd

masked va >= x, vd != v0

  pseudo-op: vsgte{u}.vx vd, va, x, v0.t
  expansion: vslt{u}.vx vd, va, x, v0.t; vmxor.vv vd, vd, v0

masked va >= x, vd == v0

  pseudo-op: vsgte{u}.vx v0, va, x, v0.t
  expansion: needs additional temp, so unsupported
----
...
vmacc.vx vd, vs2, rs1, vm    # vd[i] = vd[i] + (vs2[i] * x[rs1])

# Integer multiply-sub, overwrite minuend
vmsac.vv vd, vs2, vs1, vm    # vd[i] = vd[i] - (vs2[i] * vs1[i])
vmsac.vx vd, vs2, rs1, vm    # vd[i] = vd[i] - (vs2[i] * x[rs1])

# Integer multiply-add, overwrite multiplicand
vmadd.vv vd, vs2, vs1, vm    # vd[i] = vs2[i] + (vd[i] * vs1[i])
vmadd.vx vd, vs2, rs1, vm    # vd[i] = vs2[i] + (vd[i] * x[rs1])

# Integer multiply-sub, overwrite multiplicand
vmsub.vv vd, vs2, vs1, vm    # vd[i] = vs2[i] - (vd[i] * vs1[i])
vmsub.vx vd, vs2, rs1, vm    # vd[i] = vs2[i] - (vd[i] * x[rs1])
----
...
vwmaccu.vv vd, vs2, vs1, vm    # vd[i] = vd[i] + (vs2[i] * vs1[i])
vwmaccu.vx vd, vs2, rs1, vm    # vd[i] = vd[i] + (vs2[i] * x[rs1])

# Widening signed-integer multiply-add, overwrite addend
vwmacc.vv vd, vs2, vs1, vm    # vd[i] = vd[i] + (vs2[i] * vs1[i])
vwmacc.vx vd, vs2, rs1, vm    # vd[i] = vd[i] + (vs2[i] * x[rs1])

# Widening unsigned-integer multiply-sub, overwrite minuend
vwmsacu.vv vd, vs2, vs1, vm    # vd[i] = vd[i] - (vs2[i] * vs1[i])
vwmsacu.vx vd, vs2, rs1, vm    # vd[i] = vd[i] - (vs2[i] * x[rs1])

# Widening signed-integer multiply-sub, overwrite minuend
vwmsac.vv vd, vs2, vs1, vm    # vd[i] = vd[i] - (vs2[i] * vs1[i])
vwmsac.vx vd, vs2, rs1, vm    # vd[i] = vd[i] - (vs2[i] * x[rs1])
----
...
vfnmacc.vf vd, rs1, vs2, vm   # vd[i] = -(vs2[i] * x[rs1]) - vd[i]

# FP multiply-subtract-accumulator, overwrites subtrahend
vfmsac.vv vd, vs1, vs2, vm    # vd[i] = +(vs2[i] * vs1[i]) - vd[i]
vfmsac.vf vd, rs1, vs2, vm    # vd[i] = +(vs2[i] * x[rs1]) - vd[i]

# FP negate-(multiply-subtract-accumulator), overwrites minuend
vfnmsac.vv vd, vs1, vs2, vm   # vd[i] = -(vs2[i] * vs1[i]) + vd[i]
vfnmsac.vf vd, rs1, vs2, vm   # vd[i] = -(vs2[i] * x[rs1]) + vd[i]
----
