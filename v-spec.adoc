...
- figure out how best to optimize vlmul for systems where SLEN is less than spatial datapath width
...
strlen:
    mv a3, a0             # Save start
loop:
    vsetvli a1, x0, vsew8  # Vector of bytes of maximum length
...
unmasked va >= x
  ...
  pseudoinstruction: vsgte{u}.vx vd, va, x
  expansion: vslt{u}.vx vd, va, x; vmnand.vv vd, vd, vd

masked va >= x, vd != v0

  pseudoinstruction: vsgte{u}.vx vd, va, x, v0.t
  expansion: vslt{u}.vx vd, va, x, v0.t; vmxor.vv vd, vd, v0

masked va >= x, vd == v0

  pseudoinstruction: vsgte{u}.vx v0, va, x, v0.t, vt
...
strcpy:
      mv a2, a0             # Copy dst
loop:
    vsetvli x0, x0, vsew8   # Max length vectors of bytes
...
strncpy:
      mv a3, a0             # Copy dst
loop:
    vsetvli x0, a2, vsew8   # Vectors of bytes.
    vlbuff.v v1, (a1)       # Get src bytes
    vseq.vi v0, v1, 0       # Flag zero bytes
...
loop:
    vsetvli a5, a0, vsew32,vlmul8  # 32-bit integers
