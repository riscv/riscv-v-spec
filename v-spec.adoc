...
Known issues with current document.

- additional fixed-point arithmetic support needs to be added.
- encoding needs further cleaning/slight reorg, better formatting
- figure out how best to get effect of vlmul?max
- drop vins and add slide1up/slide1down to help debugger, reduce complexity? or move vins to OPCFG space
...
NOTE: For the zeroing tail updates, implementations with temporally
long vector registers, either with or without register renaming, will
be motivated to add microarchitectural state to avoid actually writing
zeros to all tail elements, but this is a relatively simple
microarchitectural optimization.  The microarchitectural state for an
element group indicates that zero should be returned for the element
group on a read, and that zero should be substituted in for any
masked-off elements in the group on a write (after which the element
group zero bit can be cleared).

NOTE: Not zeroing inactive elements on a masked operation reduces code
path length for many code blocks, and reduces register pressure by
allowing different code paths to use disjoint sets of elements in the
same vector register.  Implementations with vector register ECC or
renaming will have to perform read-update-write on the destination
register value to preserve inactive elements on arithmetic
instructions, so would appear to need an extra vector register read
port.  However, the arithmetic instructions are designed such that the
largest read-port requirement is for fused multiply-add instructions
that are destructive and overwrite one source, and hence do not need
an extra read port to preserve inactive elements.  Given that linear
algebra is one of the more important applications for vector units,
and that fused multiply-add is the dominant operation in linear
algebra routines, microarchitectures will be optimized for fused
multiply-add operations and so should be able to preserve inactive
elements on other arithmetic operations without large additional cost.
However, masked vector load instructions incur the cost of an
additional read port on their destination register.  The need to
support resumable vector loads with non-zero `vstart` values also
drives the need to preserve vector load destination register values.
The AMOs have been defined to be destructive in their source operand
to reduce the maximum read port requirement for the memory pipe. An
option that was considered was to have loads behave differently from
arithmetic instructions and to zero any masked-off elements. However,
this would require additional instructions and increase register
pressure, and vector loads must in any case still cope with non-zero
`vstart` values through some mechanism.
...
lumop[4:0]
   00000  unit-stride
   00sss  array-of-structures sss=(fields-1)
   10000  unit-stride first-fault
   10sss  array-of-structure first-fault sss=(fields-1)
...
The array-of-structures addressing mode is described below. [[ sec-aos ]]
...
----
 ...
 vsbc.vi   vd, vs2, imm  # Vector-immediate
----
...
masked va >= x, vd == v0
  ...
  pseudo-op: vsgte{u}.vx v0, va, x, v0.t, vt
  expansion: vslt{u}.vx vt, va, x;  vmandnot.vv v0, v0, vt
----
...
----
    # Are args ordered?
    vford.vv   # Vector-vector
    vford.vf   # Vector-scalar
----
...
----
    ...
    vfgt.v v0, va, vb, v0.t   # so only set flags on ordered values.
----

NOTE: Detailed NaN signalling conventions to be added.
...
and a scalar held in element 0 of a vector register, and performs a
...
----
    # Simple reductions.
    vredsum.vs  vd, vs2, vs1, vm   # vd[0] =  sum_i( vs2[i] ) + vs1[0]
    vredmaxu.vs vd, vs2, vs1, vm   # vd[0] = maxu_i( vs2[i] ) + vs1[0]
    vredmax.vs  vd, vs2, vs1, vm   # vd[0] =  max_i( vs2[i] ) + vs1[0]
    vredminu.vs vd, vs2, vs1, vm   # vd[0] = minu_i( vs2[i] ) + vs1[0]
    vredmin.vs  vd, vs2, vs1, vm   # vd[0] =  min_i( vs2[i] ) + vs1[0]
    vredand.vs  vd, vs2, vs1, vm   # vd[0] =  and_i( vs2[i] ) + vs1[0]
    vredor.vs   vd, vs2, vs1, vm   # vd[0] =   or_i( vs2[i] ) + vs1[0]
    vredxor.vs  vd, vs2, vs1, vm   # vd[0] =  xor_i( vs2[i] ) + vs1[0]
----
...
The `vwredsum.vv` instruction sign-extends the SEW-wide vector
...
[source]
----
    vmmv.m vd, vs  => vmand.vv vd, vs, vs   # Move mask register
    vmclr.m vd     => vmxor.vv vd, vd, vd   # Clear mask register
    vmset.m vd     => vmxnor.vv vd, vd, vd  # Set mask register
    vmnot.m vd, vs => vmnand.vv vd, vs, vs  # Invert bits
----
...
Traps on `vmiota` are always reported with a `vstart` of 0, and
...
The `vmiota` instruction can be combined with memory scatter
...
loop:
    setvli a5, a0, vsew32,vlmul8  # 32-bit integers
    vlw.v v8, (a1)                # Load input vector
      sub a0, a0, a5              # Decrement number done
      sll a5, a5, 2               # Multiply by four bytes
    ...
    vsll.vi v16, v16, 2, v0.t     # Multiply offsets by four bytes
      sll a5, a5, 2               # Multiply number of non-zero elements by four bytes
...
ignored.  If SEW > XLEN, the value is zero-extended to SEW bits.  The
other elements in the destination vector register ( 0 < index <
VLEN/SEW) are zeroed.

NOTE: Restricting the `vmv.s.x` to only write element 0 and to zero
...
NOTE: The floating-point value in `vfmv.s.f` is read from `f` register
named in `rs1` specifier, which is different than `vmv.s.x`, which
...
XLEN, the value is NaN-boxed (1-extended) to SEW bits.  The other elements in
...
elements that are read.  The start index (_OFFSET_) for the
...
----
  ...
  max(vstart, OFFSET) <= i < min(OFFSET + vl, VLMAX)    vd[i] = vs2[i-OFFSET] if mask enabled, unchanged if not
          OFFSET + vl <= i < VLMAX                      Zeroed, vd[i] = 0
----
...
`vstart` of 0.  Executing a `vmcompress` instruction with a non-zero
