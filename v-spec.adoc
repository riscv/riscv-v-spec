...
CSRs (`vtype`, `vxsat`, `vxrm`, `vl`, `vlmax`) to a base scalar RISC-V
...
0xC21    URO       vlmax  Maximum vector length
----
...
two bits `vsew[1:0]` and two bits for `vlmul[1:0]`.
...
The `vlmax` CSR is an XLEN-wide read-only register whose value is
...
NOTE: Not clear `vlmax` is required.  Can always use `vsetvl` with
AVL=x0 to get VLMAX, though this does overwrite `vl`.
...
[MLEN*_i_+(MLEN-1),MLEN*_i_] of the mask register.  When a
mask element is written by a compare instruction, the low bit in the
mask element holds the mask result and the upper bits of the mask element
are zeroed.  When a value is read as a mask, only the
least-significant bit of the mask element is used to control masking and
...
NOTE: It should be possible to specify a different form of vlmax that
specifies the range of element widths to be used in overall loop to
set LMUL correctly. TBD.
...
NOTE: Some possible further unit-stride vector addressing modes could
transfer array-of-structures formats in memory into multiple
destination vector registers.  For example, using three bits of
`*mopu` to indicate the number of SEW fields in a structure from 1-8.
...
== Vector Single-Width Floating-Point Instructions
...
vfmscc.vv vd, vs1, vs2, vm    # vd[i] = +(vs2[i] * vs1[i]) - vd[i]
vfmscc.vs vd, rs1, vs2, vm    # vd[i] = +(vs2[i] * x[rs1]) - vd[i]

# FP negate-(multiply-subtract-accumulator), overwrites addend
vfnmscc.vv vd, vs1, vs2, vm   # vd[i] = -(vs2[i] * vs1[i]) + vd[i]
vfnmscc.vs vd, rs1, vs2, vm   # vd[i] = -(vs2[i] * x[rs1]) + vd[i]
----
...
vwfmscc.vv vd, vs1, vs2, vm    # vd[i] = +(vs2[i] * vs1[i]) - vd[i]
vwfmscc.vs vd, rs1, vs2, vm    # vd[i] = +(vs2[i] * x[rs1]) - vd[i]

# FP widening negate-(multiply-subtract-accumulator), overwrites addend
vwfnmscc.vv vd, vs1, vs2, vm   # vd[i] = -(vs2[i] * vs1[i]) + vd[i]
vwfnmscc.vs vd, rs1, vs2, vm   # vd[i] = -(vs2[i] * x[rs1]) + vd[i]
----
...
several halving steps, with equivalently rounded results.
...
vmerge.vs vd, vs2, vs1, vm  # vd[i] = vm[i] ? vs1[0] : vs2[i]
...
NOTE: The `vmerge.vs` or `vmerge.vs` instruction can be used to
__splat__ a scalar value into all elements of a vector.  This is
...
== OLD TEXT: Alternative design with floating-point registers overlaid on vector registers
