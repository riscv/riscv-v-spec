...
Yunsup Lee, Guy Lemieux, Rich Newell, Albert Ou, David Patterson, Alex
Solomatnikov, Colin Schmidt, Steve Wallach, Andrew Waterman, Jim
Wilson.
...
set of active elements is all elements from `vstart` up to the current
vector length in `vl`, regardless of mask value.
...
vmerge.vs vd, vs2, rs1, v0.t  # vd[i] = v0[i].LSB ? x[rs1] : vs2[i]
...
vmerge.vs vd, v0, rs1  # vd[i] = x[rs1], pseudo-op vmv.v.s vd, rs1
vmerge.vi vd, v0, imm  # vd[i] = imm,    pseudo-op vmv.v.i vd, imm
----

An unmasked `vmerge.vv` instruction can be used to copy one vector
register group to another.  This is given a vector pseudo-instruction
`vmv.v.v vdest, vsrc` (which expands to `vmerge.vv vdest, v0, vsrc`).

An unmasked `vmerge.vs` instruction can be used to __splat__ a scalar
`x` register value into all active elements of a vector. This is given
a vector pseudo-instruction `vmv.v.s vd, rs1`, which expands to `vmerge.vs
vd, v0, rs1`.
...
pseudo-instruction `vmv.v.i vd, imm`.
...
A vector-scalar floating-point merge instruction is provided.  Unlike
regular arithmetic instructions, the set of active elements is all
elements from `vstart` up to the current vector length in `vl`
regardless of mask value.

When the floating-point merge instruction is unmasked (`vm=0`), at
...
is copied to the destination element.
...
must have the `vs2` field set to `v0` and all other values for `vs2`
are reserved.
...
This is given a vector pseudo-instruction `vmv.v.f vd, rs1` which
